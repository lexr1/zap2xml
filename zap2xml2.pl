...
%options=();
getopts("?aA:bB:c:C:d:DeE:FgH:h:i:IjJ:l:Lm:Mn:N:o:Op:P:qRr:s:S:t:Tu:Uv:wWxY:zZ:89",\%options);
...
$urlRoot = 'https://'.$options{H}.'/' if defined $options{H};
$aid = 'orbebb';
$aid = $options{h} if defined $options{h};
$urlAssets = 'https://zpmc.tmsimg.com/assets/';
...
$uastr = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/150.0.0.0 Safari/537.36";
$uastr = $options{v} if defined $options{v};
my %programs = ();
my $cp;
my %stations = ();
my $cs;
...
w/gracenote.com update https://github.com/lexr1/zap2xml	
  ...
  -H zap2it domain(default: "tvlistings.gracenote.com")
  -h aid(default: "orbebb")
  -v user-agent string
